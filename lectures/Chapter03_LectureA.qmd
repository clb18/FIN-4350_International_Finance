---
title: "Chapter 3.2 - The Balance of Payments"
author: "Prof. Barker"
date: today
format:
  html:
...
Random Quote: **When goods don’t cross borders, soldiers will.**  
...
This chapter gives you the accounting “dashboard” for the international economy. The BOP is not just a record-keeping exercise—it is the bridge between **trade flows**, **capital flows**, and the “big three” macro prices that dominate international finance: **exchange rates, interest rates, and inflation**.
...
A BOP statement is fundamentally a **statement of cash flows over an interval of time**. If you remember one practical tip for this chapter, it is:
...
Conceptually, break the BOP into two big buckets:

1. **Current account:** trade and income-like flows.
2. **Capital + financial accounts:** transactions in assets and claims.
...
1. **Goods trade** (merchandise)  
2. **Services trade**  
3. **Income** (investment income, compensation, etc.)  
4. **Current transfers** (unilateral transfers: remittances, some aid, etc.)
...
- Acquisition/disposal of **nonproduced, nonfinancial assets** (e.g., some intellectual property rights)
...
**Direct investment** is cross-border investment intended to exert meaningful influence/control over operations—often proxied by a threshold such as **10% ownership**.
...
Where:
- \(X, M\): exports and imports (goods + services)  
- \(CI, CO\): capital-account inflows/outflows  
- \(FI, FO\): financial-account inflows/outflows  
- \(FXB\): change in official reserves (conceptually, the balancing item in many regime discussions)
...
Rule of thumb:  
If the private-sector transactions don’t sum near zero, **official reserves must move**.
...
- BOP “imbalance” shows up as **currency depreciation/appreciation**, not necessarily reserve loss.

In practice, even floaters watch the current and financial accounts because they signal medium-run pressures and vulnerability.
...
- Lower real interest rates normally encourage **capital outflows** (search for yield).
...
Inflation also feeds back into the exchange rate: persistent inflation differentials tend to pressure nominal depreciation under many models (covered later).
...
The J-curve is the classic “worse before better” story. Adjustment tends to occur in three stages:
...
\[
TB = (P_X^{\$} Q_X) - (S^{\$/fc} \cdot P_M^{fc} Q_M)
\]

Where:
- \(P_X^{\$}\): export price in domestic currency  
- \(Q_X\): export quantity  
- \(P_M^{fc}\): import price in foreign currency  
- \(Q_M\): import quantity  
- \(S^{\$/fc}\): spot exchange rate (domestic per unit foreign)
...
Two frames to teach together:

- **Trade/real-side frame:** current account reflects saving vs investment and trade competitiveness.
- **Finance frame:** financial account reflects portfolio preferences, risk, expected returns, and institutional credibility.
